...
func SetMagicValue(magicNr):
	...
	match magicNr:
		...
		"44" :
			TempValues["Dam"]["ConstructionSpeedMultip"] *= 1.1
		"45" :
...
func SetResearchValue(researchNr):
	...
	match researchNr:
		...
		"93" :
			var woodTypeMultip = max(unlockedWoodTypeCount() * 1.03, 1)
	...
	match woodType:
		...
		"Spruce":
			match upgradeId:
				...
				"10":
					TempValues["Dam"]["ConstructionSpeedMultip"] *= pow(1.03, upgradeLevel)
		"Chestnut":
			match upgradeId:
				"1":
		...
		"Cedar":
			match upgradeId:
				...
				"5":
					TempValues[woodType]["StorageMultip"] *= pow(1.0075, upgradeLevel) * beavers
				"6":
					TempValues[woodType]["WcPriceMultip"] *= pow(1 - 0.07, upgradeLevel)
				"7":
